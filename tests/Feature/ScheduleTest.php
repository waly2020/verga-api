...
class ScheduleTest extends TestCase
{
    ...
    public function test_offre_depart_expiration_is_scheduled_hourly(): void
    {
        /** @var list<Event> $events */
        $events = app(Schedule::class)->events();

        $event = collect($events)->first(
            fn (Event $event) => str_contains((string) $event->command, 'offres:desactiver-depart-passes')
                || str_contains($event->mutexName(), 'offres-expire-depart'),
        );
